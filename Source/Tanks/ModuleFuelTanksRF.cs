    ...
    public partial class ModuleFuelTanks : IAnalyticTemperatureModifier, IAnalyticPreview
    {
        ...
        partial void OnLoadRF(ConfigNode node)
        {
            ...
            if (!node.TryGetValue(nameof(sPressurant), ref sPressurant))
                sPressurant = RFSettings.Instance.Pressurants.FirstOrDefault() ?? TankDefinition.tankList.First().name;
        }
        ...
        partial void OnStartRF(StartState _)
        {
            ...
            cryoTanks.Clear();
            cryoTanks.AddRange(tankList.Values.Where(t => t.maxAmount > 0 && (t.vsp > 0 || t.loss_rate > 0)));
...
#region Cryogenics
        ...
        private double CalculateTankAreaFromSphericalSubTanks() => tankList.Values.Sum(t => t.totalArea);
